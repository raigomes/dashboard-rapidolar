...
- **Número de estoque em vermelho (v1.5.2):** quando o produto tem o badge (isto é, `isEstoqueBaixo(estoque)`), o número na coluna Estoque é pintado de **`text-red-600` (`#e40014`)**, contraste **4.87:1** ✓ WCAG AA sobre branco (cor já aprovada no `docs/failures/a11y-audit.md` para variação negativa; **não usar** red-500/red-400, que falham contraste). A badge continua sendo o indicador semântico — a cor é suplemento visual, a informação de estoque baixo já é transmitida pela badge (sem quebra de acessibilidade). Sem badge → número na cor padrão do texto da célula (v1.5.2 — decisão do produto 2026-08-03)
- Visível para **admin E vendedor** (RLS não filtra o badge)
- Contraste **6.37:1** (amber-800 `#92400E` / amber-100 `#FEF3C7`) — ✓ WCAG AA
- Tokens no protótipo `dashboard.pen`: `$estbg` (`#FEF3C7` = amber-100), `$estfg` (`#92400E` = amber-800), `$estbd` (`#FCD34D` = amber-300), `$estnum` (`#E40014` = red-600, cor do número de estoque na variante "Estoque baixo")
...
> **Nota (v1.5.2):** No wireframe acima, a linha "Prod B [Est. baixo]" tem estoque `2` — o **número** `2` é exibido em `text-red-600` (`#e40014`), pois `isEstoqueBaixo(2)` é verdadeiro (produto com badge na célula Nome). Linhas sem badge mantêm a cor padrão do texto da célula.
