...
> **Versão:** 1.5.3
> **Data:** 2026-08-03
> **Changelog v1.5.3:** nit **N-01** do @reviewer (informativo, docs-only) — nota de hex/contraste do vermelho de estoque **unificada** e alinhada à realidade medida. A classe `text-red-600` do Tailwind v4 renderiza ≈ **`#E60000`** (contraste **4.81:1** ✓ WCAG AA); o token `$estnum` do protótipo = **`#E40014`** (contraste **4.87:1** ✓ WCAG AA) — **ambos ≥ 4.5:1 (AA)**, sem troca de cor de design. §1.1: contraste da variação negativa corrigido (era 4.83:1 → **4.81:1**, render do `text-red-600`; token `$estnum`/4.87:1 citado entre parênteses). §2.6: regra do número vermelho reescrita — `text-red-600` ≈ `#E60000` (4.81:1); token `$estnum` = `#E40014` (4.87:1); ambos AA; mantido o aviso de **não usar** red-500/red-400. §3.3: nota do wireframe e linha "Célula Estoque" da tabela de especificações alinhadas aos mesmos valores. Sem mudança de design nem de protótipo — `dashboard.pen` **inalterado** (formato **v2.15**, token `$estnum` já correto); a incoerência era apenas textual.
> **Changelog v1.5.2:** decisão do produto 2026-08-03 — quando um produto tem a badge **"Estoque baixo"**, o **número da quantidade de estoque** na coluna Estoque da tabela de `/produtos` passa a ser pintado de **vermelho** (`text-red-600` = `#e40014`, contraste **4.87:1** ✓ WCAG AA sobre branco — cor já aprovada no `docs/failures/a11y-audit.md` para variação negativa; **não usar** red-500/red-400, que falham contraste). A badge continua sendo o indicador semântico; a cor é **suplemento visual** (a informação já é transmitida pela badge, sem quebra de acessibilidade). §3.3: tabela de especificações atualizada — Célula Estoque com número em `text-red-600` **somente quando `isEstoqueBaixo(estoque)`** (produto com badge); sem badge → cor padrão do texto da célula. §2.6: acrescentada a regra do número vermelho e o token `$estnum`. `dashboard.pen` **v2.15** (formato do arquivo — inalterado, ver correção no changelog v1.5.1): variante `N7Wq` (Row Produto Estoque Baixo) — texto do `CellEstoque` `xSBdg` (`6WhtB`) passa de `fill: $fg` para `fill: $estnum` (`#E40014`); novo token `$estnum` adicionado em `variables`; linha base `N7WNt` permanece com cor normal (`$fg`); badge `esBdg` mantido no `CellNome` `zXWzV`. As 2 linhas com estoque baixo (2 e 10) herdam o vermelho do componente. Refs do protótipo permanecem **103**.
...
- Variação negativa: `text-red-600` + `▼` (seta down) — contraste **4.81:1** ✓ (render do Tailwind v4 ≈ `#E60000`; token equivalente no protótipo: `$estnum` = `#E40014`, 4.87:1 — ver §2.6)
...
- **Número de estoque em vermelho (v1.5.2; nota unificada em v1.5.3):** quando o produto tem o badge (isto é, `isEstoqueBaixo(estoque)`), o número na coluna Estoque é pintado de **`text-red-600`** — no Tailwind v4 o render é ≈ **`#E60000`** (contraste **4.81:1** ✓ WCAG AA sobre branco); no protótipo `dashboard.pen` o token correspondente é **`$estnum` = `#E40014`** (contraste **4.87:1** ✓) — **ambos ≥ 4.5:1 (AA)**. **Não usar** red-500/red-400, que falham contraste. A badge continua sendo o indicador semântico — a cor é suplemento visual, a informação de estoque baixo já é transmitida pela badge (sem quebra de acessibilidade). Sem badge → número na cor padrão do texto da célula (v1.5.2 — decisão do produto 2026-08-03)
- Visível para **admin E vendedor** (RLS não filtra o badge)
- Contraste **6.37:1** (amber-800 `#92400E` / amber-100 `#FEF3C7`) — ✓ WCAG AA
- Tokens no protótipo `dashboard.pen`: `$estbg` (`#FEF3C7` = amber-100), `$estfg` (`#92400E` = amber-800), `$estbd` (`#FCD34D` = amber-300), `$estnum` (`#E40014`, cor do número de estoque na variante "Estoque baixo" — contraste 4.87:1 ✓; ver nota acima sobre o render `#E60000`/4.81:1 do `text-red-600`)
...
> **Nota (v1.5.2):** No wireframe acima, a linha "Prod B [Est. baixo]" tem estoque `2` — o **número** `2` é exibido em `text-red-600` (render ≈ `#E60000`, contraste 4.81:1 ✓; token `$estnum` = `#E40014`, 4.87:1 — ver §2.6), pois `isEstoqueBaixo(2)` é verdadeiro (produto com badge na célula Nome). Linhas sem badge mantêm a cor padrão do texto da célula.
...
| **Célula Estoque**  | Apenas o número (`formatInteger`), alinhado à direita (`text-right`) — **sem badge** (decisão do produto 2026-08-03: preservar o alinhamento numérico da coluna). Quando `isEstoqueBaixo(estoque)` (produto com badge na célula Nome), o número é pintado de **`text-red-600`** (render ≈ `#E60000`, contraste 4.81:1 ✓ WCAG AA; token `$estnum` = `#E40014`, 4.87:1 — ver §2.6) — cor suplementar, a badge segue como indicador semântico (decisão do produto 2026-08-03, changelog v1.5.2) |
